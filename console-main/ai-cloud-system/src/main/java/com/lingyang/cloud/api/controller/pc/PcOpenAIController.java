...
@RestController
@Tag(name = "pc端-open")
@RequestMapping("/pc/open")
public class PcOpenAIController {
    ...
    @Operation(summary = "")
    @GetMapping("/instance")
    public Result<String> instance(String id) {
        cacheQueueService.addDelayQueue(CacheQueueConstant.CUSTOMER_INSTANCE_CREATE_QUERY_QUEUE_TYPE, id, 5);
    ...
    public void source(Integer number) throws Exception {
        log.info("进行账单结算");
        BillingApi billingApi = VoEngineApiClient.getBillingApi();
        // 获取昨天日期
        Date date = DateUtils.getDate(DateUtils.getNowDate(), -number, Calendar.DAY_OF_MONTH);
            ...
            switch (entity.getSourceType()) {
                ...
                case CLOUD_NETWORK -> {
                    ...
                    if (localCustomerEipIdMap.containsKey(entity.getInstanceId())) {
                        ...
                        if (entity.getChargeType() == null ) {
                            entity.setChargeType(SourceChargeTypeEnum.POSTPAID_BY_HOUR);
                        }
                    }
        ...
        instanceNoBillEntitList.forEach(entity -> {
            ...
            if (price.compareTo(BigDecimal.ZERO) != 0) {
                ...
                if (!entity.getChargeType().equals(SourceChargeTypeEnum.POSTPAID_BY_HOUR)) {
...
//        sysCustomerMapper.batchUpdateById(customerList);
        ...
        Map<SourceRegionsEnum, List<Long>> stopInstanceIdMap = insertList.stream()
                .filter(entity -> entity.getSourceType().equals(SourceTypeEnum.ECS) &&
