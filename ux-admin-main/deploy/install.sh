#!/usr/bin/env bash
# ux-admin-main 一键安装脚本
#
# 这个脚本是给 *服务器* 用的:解压交付包后跳上去执行一次,完成依赖安装、生成 .env、
# 准备 systemd 单元(可选)、首次启动。
#
# 用法:
#   ./install.sh                            # 交互式输入 BACKEND_URL/PORT
#   BACKEND_URL=http://10.0.0.10:8082 ./install.sh   # 非交互
#   BACKEND_URL=http://10.0.0.10:8082 PORT=5180 USE_SYSTEMD=1 ./install.sh

set -eu

cd "$(dirname "$0")"
DEPLOY_HOME="$(pwd)"
APP_NAME="ux-admin-main"
...
# 3. 安装运行时依赖 (express + http-proxy-middleware)
echo "[INFO] 安装运行时依赖..."
...
EOF
    ...
    echo "[OK] 已注册 systemd 单元: ${UNIT_FILE}"
