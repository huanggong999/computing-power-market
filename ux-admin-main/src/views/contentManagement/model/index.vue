<template>
  <div class="table-box">
    ...
    >
      ...
      <template #operation="row">
        <el-button
          link
          type="primary"
          @click="openPopover('edit', modelDetailApi, row.id)"
        >
          编辑
        </el-button>
        <el-button
          link
          type="danger"
          @click="removeFn(modelDeleteApi, row.id, row.name)"
        >
          删除
        </el-button>
      </template>
    ...
    <Drawer
      ...
      size="80%"
      @submit="
        submit({ addSubmitApi: modelSaveApi, editSubmitApi: modelUpdateApi })
      "
    >
      <ProForm
        ref="proFormRef"
...
const {
  ...
  resetFn,
  removeFn,
} = useTable({ api: modelPageApi, title: "模型" });

const userInfo = useUser();

const columns: ColumnProps[] = [
  { prop: "name", label: "模型名称", search: { el: "input" } },
  { prop: "intro", label: "模型简介" },
  { prop: "cover", label: "封面图", slot: true, width: 100 },
  {
  ...
  { prop: "publishUserAvatar", label: "发布者头像", slot: true, width: 100 },
...
const formColumns: IFormColumnsProps[] = [
  { label: "模型名称", prop: "name", el: "input" },
  { label: "模型简介", prop: "intro", el: "input" },
  ...
  { label: "模型状态", prop: "status", el: "radio", radioList: StatusEnum },
  { label: "模型介绍", prop: "introduce", el: "markdownEditor" },
];
watch(
  () => addOrEdit.value,
  (val) => {
