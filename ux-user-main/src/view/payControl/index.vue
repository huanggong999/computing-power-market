<template>
  <div class="pay">
    ...
    <div class="card">
      ...
      <TipText class="fwb" content="订单信息" fontSize="20" />
      <FormCreate
        class="mt28 mb28"
        :rule="payRule"
        v-model:api="fApi"
      ...
      >
        ...
        <el-table-column property="networkProductName" label="产品名称" />

        <el-table-column property="originalPrice" label="售价">
        </el-table-column>
        <el-table-column property="networkCount" label="购买个数">
        </el-table-column>
        <el-table-column property="networkDay" label="购买天数">
        </el-table-column>
        <el-table-column
          v-if="networkProduction.$state.production.bandwidth"
          property="bandwidth"
          label="宽带"
        >
        </el-table-column>
        <el-table-column property="finalPayAmount" label="订单应付金额">
        </el-table-column>
      </el-table>

      <!-- <div class="info-list">
        <div class="info-item flx-align-center">
          <div class="label">购买天数</div>
          <el-input-number v-model="orderInfo.networkDay" :min="1" :max="999" />
          <div class="label2">天</div>
        </div>
        <div class="info-item flx-align-center">
          <div class="label">购买个数</div>
          <el-input-number
            v-model="orderInfo.networkCount"
            :min="1"
            :max="999"
          />
          <div class="label2">个</div>
        </div>
        <div
          class="info-item flx-align-center"
          v-if="networkProduction.$state.production.bandwidth == 1"
        >
          <div class="label">宽带数量</div>
          <el-input-number v-model="orderInfo.bandwidth" :min="1" :max="999" />
          <div class="label2">M</div>
        </div>
      </div> -->
      <!-- 优惠券 -->
...
const networkProduction = useProduction();
// 购买表单
const fApi = ref();
const payRule = computed(() =>
  formCreate.parseJson(networkProduction.production.payFormValue.json)
);
...
// watch(
//   () => [
//     orderInfo.value.networkDay,
//     orderInfo.value.networkCount,
//     orderInfo.value.bandwidth,
//   ],
//   (
//     [newNetworkDay, newNetworkCount, newBandwidth],
//     [oldNetworkDay, oldNetworkCount, oldBandwidth]
//   ) => {
//     // 限制 networkDay 的范围
//     if (newNetworkDay < 0 || newNetworkDay > 999 || isNaN(newNetworkDay)) {
//       orderInfo.value.networkDay = oldNetworkDay
//     } else {
//       buildOrder()
//     }
...
//       newNetworkCount < 0 ||
//       newNetworkCount > 999 ||
//       isNaN(newNetworkCount)
//     ) {
//       orderInfo.value.networkCount = oldNetworkCount
//     } else {
//       buildOrder()
//     }

//     // 限制 bandwidth 的范围
//     if (newBandwidth < 0 || newBandwidth > 999 || isNaN(newBandwidth)) {
//       orderInfo.value.bandwidth = oldBandwidth
//     } else {
//       buildOrder()
//     }
//   },
...
const buildOrder = () => {
  let params: any = {
    orderType: "PRODUCT",
    networkPayValueId: networkProduction.$state.production.networkPayValueId,
    networkProductId: networkProduction.$state.production.networkProductId,
    bandwidth: networkProduction.$state.production.bandwidth
      ? networkProduction.$state.production.bandwidth
      : 1,
    networkDay: networkProduction.$state.production.networkDay,
    networkCount: networkProduction.$state.production.networkCount,
    payFormValue: networkProduction.$state.production.payFormValue,
  };
  if (networkProduction.$state.production.bandwidth == 1) {
    params.bandwidth = networkProduction.$state.production.bandwidth;
  }
  buildOrderInfoApi(params).then((res: any) => {
...
const createOrder = debounce(
  () => {
    ...
    if (networkProduction.$state.production.bandwidth == 1) {
      data.bandwidth = orderInfo.value.bandwidth;
    }
