<template>
  <el-dialog v-model="visible" title="实例续费" width="420px" :close-on-click-modal="false">
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="loading" @click="handleSubmit">确认续费</el-button>
    </template>
  </el-dialog>
</template>
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (!valid || !props.instance) return
    loading.value = true
    try {
      await instanceStore.renewInstance(props.instance.id, {
