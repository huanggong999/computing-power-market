<template>
  <div class="pay">
    ...
    <div class="card">
      ...
      <!-- <div class="order-price mt30">
        <div class="label flx-align-center">
          优惠总金额:
          <div class="discount">{{ total.discountAmount }}</div>
        </div>
        <div class="label flx-align-center">
          实付金额:
          <div class="actually">{{ Number(total.payAmount).toFixed(2) }}</div>
        </div>
      </div> -->
      <!-- 优惠券 -->
          ...
          <div class="label mr34">
            实付金额<span style="color: red"
              >￥<span style="font-weight: bold; font-size: 28px">{{
                order.$state.order.chargeType === "POSTPAID_BY_HOUR"
                  ? 0
                  : billAmount
              }}</span></span
            >
...
const routerList = ref([
  ...
  { name: "创建实例", path: "/cloud/createInstance" },
  { name: "支付订单", path: "" },
]);
// 支付卡片
const payCardList = [
  ...
  () => {
    ...
    const data = {
      ...
      onlinePayType: selectPay.value.includes("ALI_PAY")
        ...
        ? "WECHAT_PAY"
        : null,
      bandwidth: order.$state.order.bandwidth,
      orderSource: [
        {
          uid: order.$state.order.disk[0].uid,
          regionsId: order.$state.order.region,
          sourceType: "ECS",
          ...
          productType: order.$state.order.productType,
          configDetail: {
